...
def visualize_scatter(df, feat1=0, feat2=1, labels=2, weights=[-1, -1, 1],
    ...
    line = mlines.Line2D(line_start, line_end, color='gold')
    ax.add_line(line)
...
if __name__ == "__main__":
    ...
    #======== INPUT1.CSV =======#
    # print("Visualizing results1.csv")
    ...
    data = {'feat1': pd.read_csv('data2.csv').iloc[:, 0],
            'feat2': pd.read_csv('data2.csv').iloc[:, 1],
            'labels': pd.read_csv('data2.csv').iloc[:, 2]
            }
